...
The manager we use to keep track of these different groups of modules is called **npm**, short for **N**ode **P**ackage **M**anager. Think of a node package as an external library. We can run npm to install different packages, which are stored in a file called `package.json`. We'll get into installing packages when using Express.
...
**Note:** We've been using the `-g` flag when installing using npm so far. `-g` installs the package globally. You'll want to reserve `-g` for packages that need to be run in the command line.
